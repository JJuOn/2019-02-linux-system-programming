...
void Producer(void *dummy) {
	...
	for (i = 0 ; i < NLOOPS ; i++)  {
		...
		while (Buf.counter == MAX_BUF)  {
			// Wait condition variable notfull
		...
		Buf.counter++;
		// Signal condition variavle notempty
...
void Consumer(void *dummy){
	...
	for (i = 0 ; i < NLOOPS ; i++)  {
		...
		// If buffer is empty
		while (Buf.counter == 0)  {
			// Wait condition variable notempty
		...
		Buf.counter--;
		// Signal condition variable notfull
